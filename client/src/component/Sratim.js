        ...
        setFavoriteGenreMoviesList(favoriteGenreMovies && favoriteGenreMovies?.map((item, index) => {
            return (
                    <div key={index} className="img-container">
                        <img src={`http://localhost:5000/movies/photo/?photo_src=${item.photo_src.split('./Media/Movies-Photos/')[1]}`} />
                        <div class="img-title-bottom-left">{item.title}</div>
                        {/* <div class="img-bottom-left">Release Date: {moment.utc(item.publish_Date).format('MM/DD/YY')}</div> */}
                        {/* <div class="img-bottom-left">👍🏼 {item.likes}</div> */}
                        {/* <div class="img-bottom-left">⭐ {item.rate}</div> */}
